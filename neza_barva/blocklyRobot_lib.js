	...
	var localLanguageStrings = {
		fr: {
         ...
         messages: {
            ...
            leavesGrid: "Neža bi barvala izven pobarvanke! To ne sme!",
			 ...
			 downOutsideGrid: "Neža bi barvala izven pobarvanke! To ne sme!",
		...
		sl: {
			...
			label: {									//Imena ukazov
				...
				paint: "postavi oviro",
				//custom HEXAGONAL
				hex_east: "premakni voščenko desno",
				hex_west: "premakni voščenko levo",
				hex_noea: "premakni voščenko desno-gor",
				hex_nowe: "premakni voščenko levo-gor",
				hex_soea: "premakni voščenko desno-dol",
				hex_sowe: "premakni voščenko levo-dol",
				hex_move: "%1",
				paint2: "pobarvaj rdeče",
			...
			code: {
				...
				hex_east: "pojdi esno",
				hex_west: "pojdilevo",
				hex_noea: "pojdidesno-gor",
				hex_nowe: "pojdilevo-gor",
				hex_soea: "pojdidesno-dol",
				hex_sowe: "pojdilevo-dol",
				hex_move: "pojdiali",
				paint2: "pobarvajpolje",
			...
			description: {								//Opis ukazov
				...
				paint: "pobarva",
				//custom HEXAGONAL
				hex_east: "pojdi desno",
				hex_west: "pojdi levo",
				hex_noea: "pojdi desno-gor",
				hex_nowe: "pojdi levo-gor",
				hex_soea: "pojdi desno-dol",
				hex_sowe: "pojdi levo-dol",
				hex_move: "%1",
				paint2: "pobarvaj polje",
			...
			messages: {								//Besedila sporočil
				...
				successMarkersPainted: "Neža je zelo vesela. Kako lepa rožica!",
				failureMarkersPainted: "Neža ni pravilno pobarvala vseh polj. Poskusi še enkrat.",
				...
				leavesGrid: "Voščenka je izven mreže!",
...
//      context.blocklyHelper.updateSize();
   ...
   function createItem(newItem) {
      ...
      if (isOutsideGrid(newItem.row, newItem.col)) {
         throw("Neža bi barvala izven pobarvanke! To ne sme!");
      }
   ...
   context.robot.south = function(callback) {
	...
	//custom HEXAGONAL
	context.robot.hex_east = function(callback) {
		if (context.lost) {
			return;
		}
		var item = context.getRobotItem(context.curRobot);
		if (!checkTileAllowed(item.row, item.col + 2)) {
			context.waitDelay(callback);
		}
		else {
			context.nbMoves++;
			moveRobot(item.row, item.col + 2, 0, callback);
		}
	};
	context.robot.hex_west = function(callback) {
		if (context.lost) {
			return;
		}
		var item = context.getRobotItem(context.curRobot);
		if (!checkTileAllowed(item.row, item.col - 2)) {
			...
			context.nbMoves++;
			moveRobot(item.row, item.col - 2, 2, callback);
		}
	};
	context.robot.hex_noea = function(callback) {
		if (context.lost) {
			return;
		}
		var item = context.getRobotItem(context.curRobot);
		if (!checkTileAllowed(item.row - 2, item.col + 1)) {
			context.waitDelay(callback);
		}
		else {
			context.nbMoves++;
			moveRobot(item.row - 2, item.col + 1, 3, callback);
		}
	};
	context.robot.hex_nowe = function(callback) {
		if (context.lost) {
			return;
		}
		var item = context.getRobotItem(context.curRobot);
	...
	context.robot.hex_soea = function(callback) {
		if (context.lost) {
			return;
		}
		var item = context.getRobotItem(context.curRobot);
		if (!checkTileAllowed(item.row + 2, item.col + 1)) {
			...
			context.nbMoves++;
			moveRobot(item.row + 2, item.col + 1, 1, callback);
		}
	};
	context.robot.hex_sowe = function(callback) {
		if (context.lost) {
			return;
		}
		var item = context.getRobotItem(context.curRobot);
	...
	context.robot.hex_move = function(status, callback) {
		if(status == "west"){
			context.robot.hex_west(callback);
		}
		else if(status == "nowe"){
			context.robot.hex_nowe(callback);			
		}
		else if(status == "noea"){
			context.robot.hex_noea(callback);			
		}
		else if(status == "sowe"){
			context.robot.hex_sowe(callback);			
		}
		else if(status == "soea"){
			context.robot.hex_soea(callback);			
		}
		else{
			context.robot.hex_east(callback);			
		}

		context.waitDelay(callback);
	}			
	
	function paint2(row, col, paintType){
		var newItem = {row: row, col: col, type: paintType};
		var paintItems = context.getItems(row, col, {isPaint: true});
		if ((paintItems.length != 0) && (paintItems[0].type != paintType)) {
			destroyItem(row, col, paintType);
			paintItems.splice(0, 1);
		}
		if (paintItems.length == 0) {
			createItem(newItem);
	...
	context.robot.paint2 = function(callback) {
		var item = context.getRobotItem(context.curRobot);
		paint2(item.row, item.col, "paint");
		paint2(item.row-1, item.col-1, "paint1");
		paint2(item.row-1, item.col, "paint2");
		paint2(item.row-1, item.col+1, "paint3");
		paint2(item.row, item.col-1, "paint4");
		paint2(item.row, item.col+1, "paint6");
		paint2(item.row+1, item.col-1, "paint7");
		paint2(item.row+1, item.col, "paint8");
		paint2(item.row+1, item.col+1, "paint9");
   ...
   context.customBlocks = {
      robot: {
         actions: [
            ...
            { name: "hex_sowe" },
            { name: "hex_soea" },
            { name: "hex_move", blocklyJson: {"args0": [{"type": "field_dropdown", "name": "PARAM_0", "options": [
				[localLanguageStrings[window.stringsLanguage]["label"]["hex_east"],"east"],
				[localLanguageStrings[window.stringsLanguage]["label"]["hex_west"],"west"],
				[localLanguageStrings[window.stringsLanguage]["label"]["hex_nowe"],"nowe"],
				[localLanguageStrings[window.stringsLanguage]["label"]["hex_noea"],"noea"],
				[localLanguageStrings[window.stringsLanguage]["label"]["hex_sowe"],"sowe"],
				[localLanguageStrings[window.stringsLanguage]["label"]["hex_soea"],"soea"],
			]}]}},
            { name: "paint2" },			
