var getContext = function(display, infos, curLevel) {
    var localLanguageStrings = {
        ...
        sl: {
            ...
            messages: {
                ...
                successMarkersPainted: "Čestitamo, piščanček Pišček je pravilno postavil gnezda!",
                failureMarkersPainted: "Piščanček Pišček ni postavil gnezda na pravo polje.",
                ...
                leavesGrid: "Piščanček Pišček je zapustil mrežo!",
            ...
            changeRobotString: "Zamenjaj robota: %1",
            noRobot: "Robota ni na mreži."
        },
        none: {
            comment: {}
        }
    ...
    /*context.robot.sensorOn = function(id, callback) {
        var robot = context.getRobotItem(context.curRobot);

    };

    var findPath = function(startRow, startCol, endRow, endCol) {
        var path = [];
        var search = [[1, 0], [0, 1], [-1, 0], [0, -1]];
        var cells = [[startRow, startCol]];
        while (cells.length > 0) {
            var [row, col] = cells.shift();
            if ((row == endRow) && (col == endCol)) {
                break;
            }
            for (var step in search) {
                var newRow = row + step[0];
                var newCol = col + step[1];
                if ((0 <= newRow) && (newRow < context.tiles.length) && (0 <= newCol) && (newCol < context.tiles[0].length)) {
                    var items = context.getItems(newRow, newCol, {category: "obstacle"});
                    if ((items.length == 0) && !([newRow, newCol] in cells)){
    ...
    context.robot.turnToItem = function(id, callback) {
        var robot = context.getRobotItem(context.curRobot);
        var items = context.getItems(undefined, undefined, {catgeroy: id});

    };

    context.robot.changeRobot = function (id, callback) {
        var allRobots = context.getItems(undefined, undefined, {category: "robot"});
        context.lost = true;
        for (var iRobot = 0; iRobot < allRobots.length; iRobot++) {
            if (id == allRobots[iRobot].type) {
                context.curRobot = iRobot;
                context.lost = false;
            }
        }
        if (context.lost) {
            throw(strings.noRobot);
        }
        context.callCallback(callback);
    };
    */
