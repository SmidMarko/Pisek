...
var getContext = function(display, infos, curLevel) {
    var localLanguageStrings = {
        ...
        sl: {
            label: {
                ...
                writeToScreen: "izpiši",
                forwardAmount: "pojdi naprej za",
                turnLeft: "obrni se levo",
                turnRight: "obrni se desno",
                currentPosition: "trenutniPoložaj",
                controls_flow_statements: "ustavi potovanje",
            },
            code: {
                wait: "počakaj",
                right: "obrniSeDesno",
                ...
                writeToScreen: "izpiši",
                forwardAmount: "pojdiNaprejZa",
                turnLeft: "zavijLevo",
                turnRight: "zavijDesno",
                currentPosition: "trenutniPolozaj",
                controls_flow_statements: "ustaviPotovanje",
            },
            messages: {
                ...
                successReachGeenArea: "Čestitke! Plesalka je priplesala do rumenega polja!", // TODO: remove
                failureReachGeenArea: "Plesalka ni priplesala do rumenega polja.", // TODO: remove
                ...
                leavesGrid: "Avto je zapustil vozišče!",
                successCar: "Čestitke! Naloga je rešena.",
                failureCar: "Ojoj! Prepotovana pot ni pravilna. Poskusi še enkrat.",
                tooFewChars1: "Vrstica ",
                tooFewChars2: " je prekratka.",
                tooManyChars1: "Vrstica ",
                tooManyChars2: " je predolga.",
                tooFewLines: "Na zaslonu v avtu za izpis je premalo vrstic.",
                tooManyLines: "Na zaslonu v avtu za izpis je preveč vrstic."
            },
            description: {
                platformAbove: "ploščadZgoraj(): Ali obstaja ploščad nad avtom?",
                obstacleInFront: "oviraSpredaj(): Ali obstaja ovira pred avtom?",
                onHole: "naLuknji(): Ali je avto na luknji?",
                onTransportable: "naPredmetu(): Ali je avto na predmetu, ki ga lahko pobere?",
                onRoundObject: "naKrogu(): Ali je avto na okroglem predmetu?",
                onSquareObject: "naKvadratu(): Ali je avto na kvadratnem predmetu?",
                onTriangleObject: "naTrikotniku(): Ali je avto na trikotnem predmetu?",
                onDottedObject: "naPikčastemPredmetu(): Ali je avto na pikčastem predmetu?",
                onFullObject: "naPolnemPredmetu(): Ali je avto na polnem predmetu?",
                onStripedObject: "naČrtastemPredmetu(): Ali je avto na črtastem predmetu?",
                paintOnCell: "pobarvanoPolje(): Ali je polje, na katerem je avto, pobarvano?",
                gridEdgeInFront: "robMrežeSpredaj(): Ali je rob mreže pred avtom?",
                gridEdgeEast: "robMrežeDesno(): Ali je rob mreže desno od avta?",
                gridEdgeWest: "robMrežeLevo(): Ali je rob mreže levo od avta?",
                gridEdgeAbove: "robMrežeZgoraj(): Ali je rob mreže nad avtom?",
                gridEdgeBelow: "robMrežeSpodaj(): Ali je rob mreže pod avtom?",
                platformInFront: "ploščadSpredaj(): Ali obstaja ploščad pred avtom?",
                numberUnder: "številkaPolja(): Številka polja, na katerem je avto.",
                col: "stolpec(): Stolpec na katerem je avto",
                row: "vrstica(): Vrstica na kateri je avto",
                paintNorth: "pobarvanaZgoraj(): Ali je pobarvano polje nad poljem, na katerem je avto?",
                paintNorthWest: "pobarvanaZgorajLevo(): Ali je pobarvano polje levo nad poljem, na katerem je avto?",
                paintNorthEast: "pobarvanaZgorajDesno(): Ali je pobarvano polje desno nad poljem, na katerem je avto?",
                writeNumber: "zaPiščiŠtevilko(številka): ZaPišče številko v polje, na katerem je avto."
            },
            obstacle: "Avto poskuša premakniti oviro!",
            startingBlockName: "prepotovanaPot",
            exit_grid: "Avto je zapustil mrežo!",
            nothingToPickUp: "Ni predmetov, ki bi jih lahko pobral.",
            alreadyTransportingObject: "Avto že nosi predmet.",
            ...
            dropWithoutItem: "Avto skuša spustiti predmet, vendar ga ne nosi.",
            ...
            noBlueCell: " ni na rdečem polju.",
            forwardAmountString: "pojdi naprej za %1",
        },
        none: {
            comment: {}
        }
    ...
    var rowsLabels = [];
    var screen = {};
    var rowToLetter = "ABCDEFGHIJKLMNOPRSTUVWXYZ";
    var scale = 1;
    var paper;
    ...
    context.robot.row = function (callback) {
        var item = context.getRobotItem(context.currentRobot);
        context.callCallback(callback, rowToLetter[item.row]);
    };
    ...
    context.robot.forwardAmount = function (value, callback) {
        if (context.lost) {
            return;
        }
        var item = context.getRobotItem(context.currentRobot);
        ...
        for (var i = 0; i < value; i++) {
            var coords = getCoordsInFront(dDir);
            if (!checkTileAllowed(coords.row, coords.col)) {
                context.waitDelay(callback);
            }
            if (infos.hasGravity) {
                ...
                context.nbMoves++;
                context.grid[coords.row][coords.col] = 2;
                if (infos.pathColor != undefined && context.tiles[coords.row][coords.col] != 0) {
                    cells[coords.row][coords.col].attr({'fill': infos.pathColor});
                }
                else {
                    cells[coords.row][coords.col].attr({'fill': '#afafaf'});
                }
                moveRobot(context.currentRobot, coords.row, coords.col, item.dir, false);
            }
        }
        context.waitDelay(callback);
    };

    context.robot.turnLeft = function (callback) {
        if (context.lost) {
            return;
        }
        var item = context.getRobotItem(context.currentRobot);
        var newDir = (item.dir + 3) % 4;
        moveRobot(context.currentRobot, item.row, item.col, newDir, callback);
    };

    context.robot.turnRight = function (callback) {
        if (context.lost) {
            return;
        }
        var item = context.getRobotItem(context.currentRobot);
    ...
    var getCurrentPosition = function (iRobot) {
        var robot = context.getRobotItem(iRobot);
        var position = rowToLetter[robot.row] + (robot.col + 1);
        return position;
    };

    context.robot.currentPosition = function (callback) {
        var position = getCurrentPosition(context.currentRobot)
        context.callCallback(callback, position);
    };

    context.robot.writeToScreen = function (value, callback) {
        if (typeof callback == "undefined") {
            callback = value;
            value = "";
        }
        context.textAtScreen = value;
        context.text = (context.text != undefined) ? context.text + '\n' + context.textAtScreen: context.textAtScreen;
        resetScreen();
        context.waitDelay(callback);
    };
   ...
   context.reset = function(gridInfos) {
      if (gridInfos) {
         ...
         context.output = gridInfos.output;
         if(gridInfos.endPosition)
            context.endPosition = gridInfos.endPosition;
      }
      context.items = [];
      context.lost = false;
      context.nbMoves = 0;
      ...
      if (gridInfos) {
          ...
          for (var iRow = 0; iRow < context.nbRows; iRow++) {
              ...
              for (var iCol = 0; iCol < context.nbCols; iCol++) {
                  if (context.tiles[iRow][iCol] == 2) {
                      context.grid[iRow].push(1);
                  }
                  else {
                      context.grid[iRow].push(context.tiles[iRow][iCol]);
                  }
              }
          }
      }
...
//      context.blocklyHelper.updateSize();
   ...
   context.customBlocks = {
      robot: {
         actions: [
            ...
            { name: "writeToScreen", params: [null]},
            { name: "forwardAmount",
                blocklyJson: {
                    "message0": strings.forwardAmountString,
                    "args0": [
                        {"type": "input_value",
                            "name": "PARAM_0",
                            "check": "Number",
         ...
         sensors: [
            ...
            { name: "currentPosition",    yieldsValue: true },
         ]
      },
      transport: {
         sensors: [
   ...
   var resetBoard = function() {
       for(var iRow = 0;iRow < context.nbRows;iRow++) {
           ...
           for(var iCol = 0;iCol < context.nbCols;iCol++) {
               ...
               if(infos.showPath) {
                   if(infos.pathColor && context.tiles[iRow][iCol] == 2)
                       cells[iRow][iCol].attr({'fill': infos.pathColor});
                   if(infos.pathColor == undefined && context.tiles[iRow][iCol] == 2)
                       cells[iRow][iCol].attr({'fill': '#afafaf'});
               }
           }
       }
       if(infos.showLabels) {
           for(var iRow = 0;iRow < context.nbRows;iRow++) {
               rowsLabels[iRow] = paper.text(0, 0, rowToLetter[iRow]);
           }
           for(var iCol = 0;iCol < context.nbCols;iCol++) {
   ...
   var resetScreen = function() {
       ...
       if(screen.background != undefined) {
           screen.background.remove();
           screen.display.remove();
           screen.title.remove();
           screen.text.remove();
       }
       screen.background = paper.rect(x, y, 300, 100).attr({'fill': '#bfbfbf', 'fill-opacity': 0.5, 'r': 8, 'stroke-width': 1});
       screen.display = paper.rect(x+20, y+20, 260, 60).attr({'fill': '#ffffff', 'stroke': '#005bbf', 'stroke-width': '5', 'fill-opacity': 0.9});
       screen.title = paper.text(x+10, y+10, 'Zaslon v avtu za izpis:').attr({'font-family': 'Helvetica', 'font-size': 14, 'font-weight': 'bold', 'text-anchor': 'start'});
       screen.text = paper.text(x+50, y+50, context.textAtScreen).attr({'font-size': 22, 'font-family': 'Helvetica monospaced', 'text-anchor': 'start'});
   };
...
var robotEndConditions = {
   ...
   checkMarblesInHoles: function(context, lastTurn) {
    ...
    checkCarPath : function(context, lastTurn) {
       var solved = true;
       var printedText = (context.text != undefined) ? context.text.replace(/\s*$/, "\n"): "\n";
       var expectedText = context.output.replace(/\s*$/, "\n");

       var printedLines = printedText.split("\n");
       var expectedLines = expectedText.split("\n");

       var iLine = 0;

       if (printedLines.length < expectedLines.length) {
           solved = false;
           context.success = false;
           throw(window.languageStrings.messages.tooFewLines);
       }

       if (printedLines.length > expectedLines.length) {
           solved = false;
           context.success = false;
           throw(window.languageStrings.messages.tooManyLines);
       }

       for (iLine = 0; iLine < expectedLines.length && iLine < printedLines.length; iLine++) {
           var expectedLine = expectedLines[iLine].replace(/\s*$/, "");
           var printedLine = printedLines[iLine].replace(/\s*$/, "");

           if (printedLine.length < expectedLine.length) {
               solved = false;
               context.success = false;
               throw(window.languageStrings.messages.tooFewChars1 + (iLine + 1) + window.languageStrings.messages.tooFewChars2);
           }
           if (printedLine.length > expectedLine.length) {
               solved = false;
               context.success = false;
               throw(window.languageStrings.messages.tooManyChars1 + (iLine + 1) + window.languageStrings.messages.tooManyChars2);
           }

           var iChar = 0;
           for (iChar = 0; iChar < expectedLine.length && iChar < printedLine.length; iChar++) {
               if (printedLine[iChar] != expectedLine[iChar]) {
                   solved = false;
                   context.success = false;
                   var errorstring = "Izpis v avtomobilu je napačen v vrstici " + (iLine + 1) + ".<br>Pravilen izpis: \"<b>" + escapeHtml(expectedLine) + "</b>\",<br>Vaš izpis: \"<b>" + escapeHtml(printedLine) + "</b>\".";
                   throw(errorstring);
               }
           }
       }
       ...
       for (var iRow = 0; iRow < context.nbRows; iRow++) {
           for (var iCol = 0; iCol < context.nbCols; iCol++) {
               if (context.grid[iRow][iCol] != context.tiles[iRow][iCol]) {
                   if (context.tiles[iRow][iCol] != 3)
                       solved = false;
               }
           }
       }
       ...
       if (context.endPosition != undefined) {
           var car = context.getRobotItem(context.currentRobot);
           if (context.endPosition.row != car.row || context.endPosition.col != car.col) {
               solved = false;
           }
       }
       ...
       if (solved) {
           context.success = true;
           throw(window.languageStrings.messages.successCar);
       } else {
           context.success = false;
           throw(window.languageStrings.messages.failureCar);
       }
       if (lastTurn) {
           context.success = false;
           throw(window.languageStrings.messages.failureCar);
