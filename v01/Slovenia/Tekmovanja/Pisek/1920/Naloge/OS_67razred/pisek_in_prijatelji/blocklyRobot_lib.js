	...
	var localLanguageStrings = {
		...
		sl: {
			...
			label: {									//Imena ukazov
				...
				nowe: "premakni se gor in levo",
				noea: "premakni se gor in desno",
				sowe: "premakni se dol in levo",
				soea: "premakni se dol in desno",
				paint: "postavi oviro",
			...
			messages: {								//Besedila sporočil
				...
				successReachGeenArea: "Super! Pišek je končal na svojem začetku.",
				failureReachGeenArea: "Ojoj! Pišek ni prispel do svojega začetnega položaja.",
				...
				//custom
				forbiddenMove: "Pazi, Pišek ne sme zapustiti črt!",
				successCounters: "Bravo Pišek je obiskal vse svoje prijatelje točno enkrat!",
				failureCounters: "Ojoj. Pišek ni obiskal vseh svojih prijateljev točno enkrat!",
			},
		},
		none: {
			comment: {
...
//      context.blocklyHelper.updateSize();
   ...
   context.robot.east = function(callback) {
      ...
      var item = context.getRobotItem(context.curRobot);
		if ((getAllowed()).indexOf("east") == -1) {
			throw(context.strings.messages.forbiddenMove);
			context.waitDelay(callback);
        }
   ...
   context.robot.south = function(callback) {
	...
	//custom blocks
	context.robot.nowe = function(callback) {
		if (context.lost) {
			return;
		}
		var item = context.getRobotItem(context.curRobot);
		if ( (getAllowed()).indexOf("nowe") == -1) {
			throw(context.strings.messages.forbiddenMove);
			context.waitDelay(callback);
        }
		if (!checkTileAllowed(item.row - 1, item.col - 1)) {
			context.waitDelay(callback);
		}
		else {
			context.nbMoves++;
			moveRobot(item.row - 1, item.col - 1, 2, callback);
		}
	};
	context.robot.noea = function(callback) {
		if (context.lost) {
			return;
		}
		var item = context.getRobotItem(context.curRobot);
		if ( (getAllowed()).indexOf("noea") == -1) {
			throw(context.strings.messages.forbiddenMove);
			context.waitDelay(callback);
        }
		if (!checkTileAllowed(item.row - 1, item.col + 1)) {
			...
			context.nbMoves++;
			moveRobot(item.row - 1, item.col + 1, 0, callback);
		}
	};
	context.robot.sowe = function(callback) {
		if (context.lost) {
			return;
		}
		var item = context.getRobotItem(context.curRobot);
		if ( (getAllowed()).indexOf("sowe") == -1) {
			throw(context.strings.messages.forbiddenMove);
			context.waitDelay(callback);
        }
		if (!checkTileAllowed(item.row + 1, item.col - 1)) {
	...
	context.robot.soea = function(callback) {
		if (context.lost) {
			return;
		}
		var item = context.getRobotItem(context.curRobot);
		if ( (getAllowed()).indexOf("soea") == -1) {
			throw(context.strings.messages.forbiddenMove);
			context.waitDelay(callback);
        }
		if (!checkTileAllowed(item.row + 1, item.col + 1)) {
   ...
   context.robot.brownCell = function(callback) {
	...
	var getAllowed = function(){
		var robot = context.getRobotItem(context.curRobot);
		var restrictions = context.getItems(robot.row, robot.col, {hasRestriction: true});
		if (restrictions.length == 0) {
			return "";
		}
		var itemType = infos.itemTypes[restrictions[0].type];
		if ((restrictions.length > 0) && (itemType["allowed"] != undefined)) {
			return itemType["allowed"];
		}
		return "";
	};
...
*/
   ...
   var moveRobot = function(newRow, newCol, newDir, callback) {
      ...
      while (collectibles.length > 0) {
		...
		//custom COUNTERS
		var counters= context.getItems(newRow, newCol, {isCounter: true});
		if (counters.length != 0) {
			counters[0].type += 1;
			if (context.display) {
				redisplayItem(counters[0]);
			}
		}
...
var robotEndConditions = {
	...
	checkCountersValues: function(context, lastTurn) {
		var solved = true;
		var total = 0;
		for (var iRow = 0; iRow < context.tiles.length; iRow++) {
			var row = context.tiles[iRow];
			for (var iCol = 0; iCol < row.length; iCol++) {
				var counters = context.getItems(iRow, iCol, {isCounter: true});
				var counters_one = context.getItems(iRow, iCol, {isCounter: true, value: 1});
				if(counters.length != counters_one.length) solved = false;
			}
		}
		if (solved) {
			context.success = true;
			throw(window.languageStrings.messages.successCounters);
		}
		if (lastTurn) {
			context.success = false;
			throw(window.languageStrings.messages.failureCounters);
		}
	},
};
//klic knjižnic
