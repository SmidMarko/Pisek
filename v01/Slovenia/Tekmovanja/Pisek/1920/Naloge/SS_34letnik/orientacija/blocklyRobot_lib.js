	...
	var localLanguageStrings = {
		...
		sl: {
			...
			messages: {								//Besedila sporočil
				...
				leavesGrid: "Tabornik je zapustil mrežo!",
				...
				obstacle: "Ojoj! Tabornik se je zaletel v oviro. Poskusi še enkrat.",
				//CUSTOM
				successGoalValue: "Bravo, Tabornik je pravilno preštel svoje nagradne točke!",
				failureGoalValue: "Tabornik, ni pravilno preštel svojih nagradnih točk!",
			},
		},
		none: {
			comment: {
...
var robotEndConditions = {
	...
	checkValueOnGoal: function(context, lastTurn) {
		var solved = true;
		var total = 0;
		for (var iRow = 0; iRow < context.tiles.length; iRow++) {
			var row = context.tiles[iRow];
			for (var iCol = 0; iCol < row.length; iCol++) {
				var rewards = context.getItems(iRow, iCol, {color: "vert"});
				if (rewards.length > 0) {
					total += (context.getItems(iRow, iCol, {category: "number"}))[0].type;
				}
			}
		}
		for (var iRow = 0; iRow < context.tiles.length; iRow++) {
			var row = context.tiles[iRow];
			for (var iCol = 0; iCol < row.length; iCol++) {
				var goals = context.getItems(iRow, iCol, {color: "jaune"});
				if (goals.length > 0 && context.getItems(iRow, iCol, {category: "number", value: total}) == 0) {
					solved = false;
				}
			}
		}
		if (solved) {
			context.success = true;
			throw(window.languageStrings.messages.successGoalValue);
			
		}
		if (lastTurn) {
			context.success = false;
			throw(window.languageStrings.messages.failureGoalValue);
		}
	},
};
//klic knjižnic
