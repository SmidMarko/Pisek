	...
	var localLanguageStrings = {
		...
		sl: {
			...
			label: {									//Imena ukazov
				...
				paint: "postavi oviro",
				pickTransportable: "poberi balon",
				dropTransportable: "pospravi balon",
				onTransportable: "na balonu",
				...
				transportableColor: "barva balona",
			...
			messages: {								//Besedila sporočil
				nothingToPickUp: "Ni balona, ki bi ga lahko pobral.",
				alreadyTransporting: "Robot že nosi balon.",
				notTransporting: "Robot skuša spustiti balon, vendar ga ne nosi.",
				successDroppedAllObjects: "Čestitamo, robot je spustil vse balone!",
				...
				successOneMarbleInHole: "Čestitamo, balon je na pravem mestu!",
				successAllMarblesInHoles: "Čestitamo, vsi baloni so na pravih mestih!",
				failureOneMarbleInHole: "Balon ni na pravem mestu!",
				failureAllMarblesInHoles: "Vsi baloni niso na pravih mestih!",
...
var robotEndConditions = {
	...
	checkMarblesInHolesColor: function(context, lastTurn){     //Frnikole v luknjah s pravimi barvami //custom function
		const colors = {green: "vert", red: "rouge",blue: "bleu", yellow: "jaune", orange: "orange", purple: "violet", brown: "marron", gray: "gris"};
		var robot = context.getRobotItem(context.curRobot);
		var solved = true;
		for (var iRow = 0; iRow < context.tiles.length; iRow++) {
			var row = context.tiles[iRow];
			for (var iCol = 0; iCol < row.length; iCol++) {
				for(var color_key in colors){
					var marbles = context.getItems(iRow, iCol, {category: "marble", color: colors[color_key]});
					var holes = context.getItems(iRow, iCol, {category: "hole", color: colors[color_key]});
					if (marbles.length != 0) {
						if(marbles.length != 1 || holes.length != 1){
							solved = false;
						}
					}
				}
