	...
	var localLanguageStrings = {
		...
		sl: {
			...
			messages: {						//Besedila sporočil
				paintingWrong: "Želva ni narisala pravega vzorca.",
				paintingCorrect: "Želva je narisala pravi vzorec.",
				paintingFree: "Želva je izvrtala vzorec, ki si ga sprogramiral. Če ga želiš shraniti, posnemi sliko zaslona.",
			},
		},
		none: {
			comment: {
	...
	checkImagesEqual: function(context, lastTurn) {
		if (lastTurn) {
			...
			for (var i = 3; i < len; i += 4) {
				...
				//if (Math.abs(userImage.data[i-3] - solutionImage.data[i-3]) > 127 || Math.abs(userImage.data[i-2] - solutionImage.data[i-2]) > 127 || Math.abs(userImage.data[i-1] - solutionImage.data[i-1]) > 127 ) delta++;
				if ((userImage.data[i-3] != solutionImage.data[i-3]) || (userImage.data[i-2] != solutionImage.data[i-2]) || (userImage.data[i-1] != solutionImage.data[i-1]) ) delta++;
