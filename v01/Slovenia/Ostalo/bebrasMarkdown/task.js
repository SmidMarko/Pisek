function initTask() {
   ...
   var getQuestions=function(questions, isSample) {
      var htmlQuestions="<tr><td></td><td><b>Prej:</b></td><td><b>Program:</b></td><td><b>Potem:</b></td></tr>";
      for (var iQuestion=0; iQuestion < questions.length; iQuestion++) {
         var question=questions[iQuestion];
         htmlQuestions += "<tr><td><b>";
         if (isSample) {
            htmlQuestions += "Primer "+(iQuestion+1);
         } else {
            htmlQuestions += "Vprašanje "+(iQuestion+1);
         }
   ...
   grader.gradeTask=function(strAnswer, answerToken, callback, gradedLevel) {
      ...
      for (var curLevel in data) {
         ...
         messages[curLevel]="Prav imate "+nbSolved+" vprašanj"+stevilo+" od "+nbQuestions;
      };
      if (gradedLevel == null) {
