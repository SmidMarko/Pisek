...
var printerEndConditions = {
	...
	checkRandMessage1:  function(context, lastTurn) {	//custom
		if (!lastTurn) return;
		var tip = ["ustno","pisno"];
		var pogled = ["zelo grd","manj grd"];
		var ocena = ["1","2","3","4"];
		
		context.success = false;
		var actualLines = context.printer.output_text.replace(/\s*$/,"").split("\n");
		if(actualLines[0] != tip[randomArray[0]-1]) throw("Napačen tip. Generirana naključna števila uporabi po vrsti!");
		if(actualLines[1] != pogled[randomArray[1]-1]) throw("Napačen pogled. Generirana naključna števila uporabi po vrsti!");
		if(actualLines[2] != ocena[randomArray[2]-1]) throw("Napačna ocena. Generirana naključna števila uporabi po vrsti!");
		context.success = true;
		throw("Program deluje pravilno!");		
	},
};
