	...
	var localLanguageStrings = {
		sl: {
			...
			label: {									//Imena ukazov
	            ...
	            jump: "skoči",
	            down: "pojdi dol",
				east: "premakni se proti vzhodu",
				south: "premakni se proti jugu",
				west: "premakni se proti zahodu",
				north: "premakni se proti severu",
				paint: "postavi oviro",
				...
				dirUnder: "smer na polju", //custom
				dirUnderEqual: "smer na polju %1", //custom
			...
			code: {
				...
				dirUnder: "smerPolja", //custom
				dirUnderEqual: "smerPoljaEnaka", //custom
			...
			messages: {								//Besedila sporočil
				...
				nothingToPickUp2: "Ni pribora, ki bi ga lahko pobral.",
				...
				successPickedAllWithdrawables: "Čestitamo, robot je pobral ves pribor!",
				failurePickedAllWithdrawables: "Robot ni pobral vsega pribora!",
				successReachGeenArea: "Čestitamo, Zajček je našel skriti zaklad.",
				failureReachGeenArea: "Ojoj, Zajček ni prišel do skritega zaklada. Poskusi še enkrat.",
				...
				leavesGrid: "Ojoj, Zajček je zapustil mrežo in se izgubil. Poskusi še enkrat.",
				...
				obstacle: "Ojoj, Zajček se je zaletel v temačno drevo. Poskusi še enkrat.",
			},
		},
		none: {
			comment: {
...
//      context.blocklyHelper.updateSize();
   ...
   context.robot.numberUnder = function(callback) {
    ...
    //custom
	context.robot.dirUnder = function(callback) {
		var robot = context.getRobotItem(context.curRobot);
		var itemsUnder = context.getItems(robot.row, robot.col, {category: "marker"});
		if (itemsUnder.length == 0) {
			context.callCallback(callback, "undefined");
		} 
		else {
			context.callCallback(callback, infos.itemTypes[itemsUnder[0].type].direction);
		}
	};
	//custom
	context.robot.dirUnderEqual = function(status, callback) {
		var robot = context.getRobotItem(context.curRobot);
		var itemsUnder = context.getItems(robot.row, robot.col, {category: "marker"});
		if (itemsUnder.length == 0) {
			...
			context.callCallback(callback, status==infos.itemTypes[itemsUnder[0].type].direction);
		}
	};
   ...
   context.customBlocks = {
      robot: {
         ...
         sensors: [
            ...
            { name: "dirUnder",        yieldsValue: true }, //custom
			{ name: "dirUnderEqual",  //custom
				yieldsValue: true , 
				blocklyJson: {
					"args0": [
						{
							...
							"name": "PARAM_0", 
							"options": [
								[localLanguageStrings[window.stringsLanguage]["messages"]["dirNorth"],  "sever"], 
								[localLanguageStrings[window.stringsLanguage]["messages"]["dirSouth"], "jug"], 
								[localLanguageStrings[window.stringsLanguage]["messages"]["dirWest"], "zahod"], 
								[localLanguageStrings[window.stringsLanguage]["messages"]["dirEast"], "vzhod"],
							],
						},
					],
				},
