...
var getContext = function(display, infos) {
	var localLanguageStrings = {
		...
		sl: {
			...
			label: {										//Imena ukazov
				randomInteger: "naključno število med %1 in %2", //custom
				print: "izpiši",
				print_end: "izpiši",
				...
				charToNumber: "Spremeni znak v številko.",
				numberToChar: "Spremeni številko v znak.",
				asciiToChar: "Spremeni ASCII številko v znak.",
				charToAscii: "Spremeni znak v ASCII številko.",
			},
			code: {
				randomInteger: "naključnoCeloStevilo", //custom
				print: "napisi",
				print_end: "napisi",
				read: "preberi",
				...
				eof: "konecVhoda",
				charToNumber: "SpremeniZnakVStevilko.",
				numberToChar: "SpremeniStevilkoVZnak.",
				asciiToChar: "SpremeniASCIIStevilkoVZnak.",
				charToAscii: "SpremeniZnakVASCIIStevilko.",
			},
			description: {								//Opis ukazov
				randomInteger: "naključno celo število med %1 in %2", //custom
				print: "naprinta",
				read: "prebere",
			...
			messages: { 								//Besedila sporočil
				...
				outputCorrect: "Čestitamo, tvoj program deluje pravilno.",
	...
	context.reset = function(gridInfos) {
		...
		if (context.gridInfos.input) {
			context.printer.input_text = context.gridInfos.input;
		}
		context.updateScale();
	};
	...
	context.printer.commonRead = function() {
		if(context.gridInfos.freeInput && context.display) {
	...
	//custom
	context.printer.randomInteger = function(a, b, callback) {
		var rand = Math.floor(Math.random() * (Math.abs(b-a)+1))+Math.min(a, b);
		randomArray.push(rand);
		context.waitDelay(callback, rand);
	}

	context.checkOutputHelper = function() {
		var expectedLines = this.gridInfos.output.replace(/\s*$/,"").split("\n");
		...
		printer: {
			...
			math: [
				{ 
					...
					blocklyJson: {
						"args0": [
							{"type": "field_number", "name": "PARAM_0", "check":"Number", "value":1}, 
							{"type": "field_number", "name": "PARAM_1", "check":"Number", "value":2}, 
						],
						"inputsInline":"true",
					},
					yieldsValue: true,
...
var printerEndConditions = {
	...
	checkRandMessage:  function(context, lastTurn) {	//custom
		if (!lastTurn) return;
		
		context.success = false;
		var actualLines = context.printer.output_text.replace(/\s*$/,"").split("\n");
		for(var i=0; i<actualLines.length; i++){
			if(actualLines[i] != randomArray[i]) throw("Narobe. Generirana naključna števila uporabi po vrsti!");
		}
		context.success = true;
		throw("Program deluje pravilno!");		
	},
	checkRandMessage1:  function(context, lastTurn) {	//custom
		if (!lastTurn) return;
		var smer = ["leva","desna"];
		var ud = ["roka","noga"];
		var barva = ["rdeča","modra","rumena","zelena"];
		
		context.success = false;
		var actualLines = context.printer.output_text.replace(/\s*$/,"").split("\n");
		if(actualLines[0] != smer[randomArray[0]-1]) throw("Napačna smer. Generirana naključna števila uporabi po vrsti!");
		if(actualLines[1] != ud[randomArray[1]-1]) throw("Napačen ud. Generirana naključna števila uporabi po vrsti!");
		if(actualLines[2] != barva[randomArray[2]-1]) throw("Napačna barva. Generirana naključna števila uporabi po vrsti!");
		context.success = true;
		throw("Program deluje pravilno!");		
	},
};
