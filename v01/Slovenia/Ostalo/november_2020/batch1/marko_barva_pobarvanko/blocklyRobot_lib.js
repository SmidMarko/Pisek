	...
	var localLanguageStrings = {
		...
		sl: {
			...
			messages: {								//Besedila sporočil
				...
				successMarkersPainted: "Čestitamo, Marko je pravilno pobarval polja!",
				failureMarkersPainted: "Marko ni pravilno pobarval vseh polj. Poskusi še enkrat.",
...
//      context.blocklyHelper.updateSize();
   ...
   context.robot.south = function(callback) {
	...
	context.robot.hex_move = function(status, callback) {
		if(status == "west"){
			context.robot.hex_west();
		}
		else if(status == "nowe"){
			context.robot.hex_nowe();			
		}
		else if(status == "noea"){
			context.robot.hex_noea();			
		}
		else if(status == "sowe"){
			context.robot.hex_sowe();			
		}
		else if(status == "soea"){
			context.robot.hex_soea();			
		}
		else{
			context.robot.hex_east();			
		}
