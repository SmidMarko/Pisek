	...
	var localLanguageStrings = {
		...
		sl: {
			...
			label: {									//Imena ukazov
				...
				forwardAmount: "pojdi naprej za %1",
			...
			messages: {								//Besedila sporočil
				...
				alreadyTransporting: "Miška že nosi predmet.",
				notTransporting: "Miška skuša spustiti predmet, vendar ga ne nosi.",
				successDroppedAllObjects: "Čestitamo, miška je spustila vse predmete!",
				successMarkersPainted: "Čestitamo, miška je pravilno postavila ovire!",
				failureMarkersPainted: "Miška ni postavila ovir na pravo polje.",
				successPickedAllCollectibles: "Čestitamo, miška je pobrala vse sire!",
				failurePickedAllCollectibles: "Miška je pobrala vse sire!",
				successReachGeenArea: "Čestitamo, Martin je prišel do gradu.",
				failureReachGeenArea: "Ojoj, Martin ni našel gradu. Poskusi še enkrat.",
				...
				leavesGrid: "Miška je zapustila mrežo!",
				jumpOutsideGrid: "Miška skuša skočiti izven mreže!",
				jumpObstacleBlocking: "Miška skuša skočiti, vendar zaradi ovire ne more!",
				jumpNoPlatform: "Miška skuša skočiti, vendar ni nobene ploščadi!",
				downOutsideGrid: "Miška se skuša premakniti izven mreže!",
				downNoPlatform: "Miška se skuša premakniti dol, vendar spodaj ni ploščadi!",
				falls: "Miška pada v prazno.",
				willFallAndCrash: "Miška bo padla s ploščadi in se poškodovala!",
				obstacleOnCell: "Na tem polju je ovira.",
				platformOnCell: "Na tem polju je že ploščad.",
				obstacle: "Ojoj, Martin se je zaletel v oviro. Poskusi še enkrat.",
			},
		},
		none: {
			comment: {
...
//      context.blocklyHelper.updateSize();
   ...
   context.robot.forward = function(callback) {
	...
	context.robot.forwardAmount = function(value, callback) {
		...
		var item = context.getRobotItem(context.curRobot);
		var coords = getCoordsInFrontAmount(0, value);
		if (!checkTileAllowed(coords.row, coords.col)) {
			context.waitDelay(callback);
		}
		if (infos.hasGravity) {
...
*/
   ...
   //custom var
	var getCoordsInFrontAmount = function(dDir, value) {
		var item = context.getRobotItem(context.curRobot);
		var lookDir = (item.dir + dDir + 4) % 4;
		return {
			row: item.row + delta[lookDir][0]*parseInt(value),
			col: item.col + delta[lookDir][1]*parseInt(value),
		};
	};
   ...
   context.customBlocks = {
      robot: {
         actions: [
			...
			{ name: "forwardAmount",
                blocklyJson: {
                    "message0": strings.label.forwardAmount,
                    "args0": [
                        {"type": "input_value",
                            "name": "PARAM_0",
                            "check": "Number",
